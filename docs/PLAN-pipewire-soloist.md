...
| AM-77 | **AM-42's `cache: N`, stated:** playlist/album/artist with N > 0 = the whole list under the ≤ 100/pass bound; show N = the newest N episodes (Soloist's `play show` order vs "newest" — bench line). The byte budget from `spotify_cache_gb` and a hotspot byte cap (~4 MB × items) go into the numbers. The POST carries `{uri, budget_s, max_items, max_bytes}`; the sidecar cannot see the battery, so the daemon's 5 s poll aborts below the threshold — **owner to set the number** (proposal: plugged, or ≥ 30 %). | C2, Q22, Q23 |
