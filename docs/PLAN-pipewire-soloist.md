...
| 5 | **5a done**, 5b/5c owed | 5a: `POST /soloist/configure` (token + JSON gates; `soloist.env` KEY=VALUE 0600; unit restarted `--no-block`; empty key removes; 409 under go-librespot; never logged), `/status.spotify_state`, `play()` fast-fails `spotify-<state>` on needs-key/needs-pair/expired/bad-key/audio-unbound, the PWA key form with the local confirm, soloistd's bad-key net (exact line bench-owed). Owed: the SCREEN popup for these states (ui.py, mockup first); 5b `--pair` via a oneshot unit surfaced in the PWA; 5c the updater unit + monotonic timer + the shutdown hook's second slot (D1/AM-50..52) with the 15-line crc32c. |
