...
| AM-58 | **One volume cap (owner 2026-09-05, first Zero).** `local_fallback_cap` (the built-in-only landing cap, §F/NEW-1/AM-7) is removed from settings, PWA and screen; `volume_cap` — the child-safety ceiling the knob already obeyed — is applied at every landing on every output (`output.local_volume(stored, cap)`; `_apply_box_volume`, mpv start, live retarget, `_go_volume_cap`). AM-7's "cap everywhere on fail-safety" collapses into this: the cap is everywhere by construction, the self-test verdict stays informational in `/status.audio_policy`. Trigger: with `local_fallback_cap = 0` ("no limit") under a fail-safety verdict, `volume()` computed `min(volume_cap, 0)` and drove the knob to 0 ("no limit behaves as 0"). |
| AM-59 | **Soloist query commands are not acked — and the Zero's numbers (2026-09-05, `bench/soloist_probe.py`).** `get_auth_state`/`get_state`/`get_queue` answer with their event only, no `command_result` (docs; probe: `queue_changed` in 0.04 s, twice). The sidecar's `cmd()` waited `CMD_TIMEOUT_S` for an ack that never comes, so every listing took the full timeout and the Sonos hand-off's `/context/tracks` overran the daemon's 5 s (`TimeoutError`) while the same ask over curl 'worked' (curl has no timeout). `cmd()` now returns at once for queries; `listing()` waits `LISTING_WAIT_S` for `queue_changed`, serves the last good listing when it does not come, else `ready=false`. Also from the probe: Soloist accepts a second WebSocket client; `previous` is capped at 10 even with `limit=0` (a 22-track playlist listed as 10 played + current + 6 upcoming = 17), so the picker for a long-played list shows the last ten played, not the whole list — a Soloist limit, recorded, not worked around. |
| AM-60 | **The list remembered at a context's start (owner 2026-09-05: 'jeg liker din nr 2').** Soloist's `previous` is 10 deep, so a window taken mid-list has lost the first tracks. At the moment a context starts — a plain play or the resume walk's `play uri`, BEFORE its skips — `previous` is empty and `upcoming` is the rest, so the sidecar snapshots it there (`_snapshot_listing`, one bounded `get_queue`) and `listing()` serves the remembered list, merging in tracks a later window shows that were not seen yet (lists longer than the window grow as they play). A fresh start (empty `previous`) re-seeds; shuffle does NOT touch it — the remembered list is the playlist in its own order, which is what a picker should show whatever the play order (the Spotify app does the same); removals are not tracked (Soloist's broadcast `queue_changed` is 10 entries — additions are merged at the next ask, e.g. the hold on Y). Repeat-context as a trick to see the whole list was considered and rejected: the list would never end, and the idle auto-off with it. |
| AM-61 | **The Soloist entity shape, verbatim from the Zero (2026-09-05, `bench/soloist_probe.py --raw`).** Context: `{uri, entity_type: "playlist", decorations.identity.name}` — **no revision/snapshot anywhere**, so the library's playlist snapshot gate (fork `/cache/snapshot`) cannot be rebuilt on Soloist without the Web API; it stays fail-open. Item: `{uri, entity_type: "track", decorations: {identity.name, visual_identity.cover[ONE entry, size "small", url `ab67616d00004851<hash>` = 64 px], parent.entity{uri, entity_type album, decorations.identity.name}, creators[].entity{uri, entity_type artist, decorations.identity.name}, playback{duration_ms, content_ratings[]}}}`. Queue entry: `{uid (16 hex), source "context", item}`. `playback_state` also carries `position{position_ms, timestamp_ms, speed (0 when paused)}`, `volume`, `is_active`, `options{shuffle, repeat, playback_speed, modes.context_enhancement}`, `available_actions.seek_backward/forward.step_ms = 15000`. Cover: the CDN id's size prefix is swapped to `b273` (640 px) in `entity_to_track` — the same hash at 640 px answered 200 / 85 KB. `previous` capped at 10 confirmed twice more. |
...
| Owed | Who |
|---|---|
| 4d warming (AM-36..45, decided in §4d.6 as AM-62..81) | **step 2 BUILT 2026-09-05 night: the sidecar core** — `warm_start`/`_warm_run`/`_warm_one`/`_dwell` on `vibb_null` with a generation-checked child, ONE `_restart_for`, frozen-as-stopped `/status`, ledger + order + meta stores (tmp+fsync+replace), size-based dwell with the baseline taken before the fetch, abort-first `/player/*`, `/cache/abort`; pinned in `tests/soloist_warm.py` W1–W9 against the shared fake in `tests/soloist_fakes.py` (a scripted cache writer: fast/slow/stall/already-cached, one-block prefetch that the real file grows from). **Step 3 BUILT 2026-09-06 00:xx:** the sweeper's soloist branch stays with the pass (`library._warm_entry`: POST, 5 s poll touching BUSY + `vibb-warming`, `_busy()` → `/cache/abort`, `_precache_done` only on `done`; AM-45 default `cache: 1`), `radio.touch_warming/warming` + btwatchd's yield holds on it before the belt, `daemon._warming()` (5 s cached health, expired deadline = none) → `/status.warming`, `ORCH.play` aborts the pass and waits BEFORE `_kick_bt_connect`, `_PS_KICK` at pass start, Sonos `sonos_map_trusted=False` on a stale listing, idle.py `warming_hold()` with the 25 min hard release, `backup._box_busy` counts warming, `accept_spot_bookmark` rejects a track no longer in the remembered list. Pinned: `sweep_warm_gate.py`, `warm_status.py`, `bt_radio_yield` 5, `idle_shutdown` 10. **Step 4 owed: the Zero** — add a list, watch the pass, wifi off, play. |
...
### 4d.6 QA + architect pass, 2026-09-05 night — decisions (AM-62..AM-80)

Two independent read-only passes over 4d.0–4d.5 against the code (architect
A1–A13, QA Q1–Q28). Verdict of both: **not implementation-ready as drafted**;
the silence premise was false in code, the kid's play button during a pass
was dead, and the library's own state machine would mark albums done on the
202. Everything below is DECIDED and supersedes the draft where it conflicts.
The draft's numbered questions Q1–Q5 are answered inline.

| # | Decision (what the code will do) | From |
|---|---|---|
| AM-62 | **The child is started on `vibb_null` through an explicit override, and every restart path goes through ONE `_restart_for(reason)`.** `start_child` today derives the node from output.json unconditionally (`soloistd.py` `self.node = self._node_for(self.current_pcm())`), so "warm on vibb_null by construction" was false, and three paths — the crash restart in `_wait_child`, `POST /player/output`, `POST /soloist/pair` — would hand the child back to the REAL sink under a running pass, after which the pass's next `play` is audible at session volume. Fix: `warm_node` override honoured by `start_child`; a child **generation counter**; the pass checks generation before EVERY command and aborts on change; `_restart_for` sets abort, joins the pass thread, then restarts onto the real node. | A1, Q1 (BLOCKER) |
| AM-63 | **Silence belts on top of the node:** before a pass, `vibb_null` must be present in `pw_dump()` else the pass is skipped; `set_volume 0` before the pass's first `play` (the resume walk's own shroud); the authoritative bind check runs 0.5 s after the first playing event and requires `sinks == {"vibb_null"}` — anything else aborts the pass and sets `audio-unbound`. A restore that fails leaves `node == vibb_null` → `_derive_state` reports `audio-unbound`, never `ok`. A `warm-active` marker in the state dir is deleted (and logged) by every sidecar start, so a sidecar crash mid-pass cannot leave a silent box that claims `bound`. `health.warming` carries a deadline; an expired flag reads as false. | Q2, Q3, A12 |
| AM-64 | **The frozen `/status` is honest: `stopped: true`, last track kept, `paused: false`, `play_origin` unchanged, `spotify_state` LIVE.** The restart onto `vibb_null` ends the kid's session anyway; a frozen `paused` made the daemon's in-place `resume` shortcut un-pause the WARM child on the null sink — a dead button. With `stopped` the daemon takes the bookmark walk (the proven post-reboot path). The freeze is installed BEFORE the null restart and lifted only after the restored child reports `idle` — otherwise the arbiter sees the warm's context as a phone takeover for a beat and kills the kid's podcast. `bookmark_step` returns None on stopped, so no bookmark poison (AM-36's intent, corrected wording). | A3, Q5, Q7, Q9 |
| AM-65 | **Every play-class `/player/*` during a pass = abort → join → restore → execute.** `play/resume/playpause/next/prev/seek` set the abort event, wait for the pass to yield (bounded), restart on the real node, wait for the WebSocket (≤10 s else 503), then run under `ENGINE.lock`. `pause` → 200 no-op; `volume`/`shuffle_context` → stored only. The pass child is terminated with kill after 1 s (not the 5 s grace). Target to beat: the daemon's 8 s play timeout with its "did it land" recheck. Under soloist `ORCH.play` POSTs `/cache/abort` and waits for `warming == false` BEFORE `_kick_bt_connect()` — a BT page under a fetching pass is the thrice-observed flap. | A2, Q5, Q6, Q25 |
| AM-66 | **Locking:** the pass thread takes `ENGINE.lock` per step (one `cmd` + its wait), never across a dwell; re-checks abort + generation under it. `cmd()`'s ack correlation by verb is otherwise crossed by a daemon POST of the same verb. `/context/tracks` for the warm context is serialized under the same lock. | A2, Q25, Q26 |
| AM-67 | **The gate stays `_busy()` = audible (today's `BUSY_CHECK`); AM-38's "audible AND streaming" relaxation is DROPPED.** Two reasons from the code: the sweeper's own BUSY touch makes `_streaming_now()` true, collapsing the relaxed gate anyway; and relaxing it would run the podcast/storytel downloads under audible cached A2DP — the coex leg. The sweeper's soloist branch polls every 5 s (touch BUSY + `vibb-warming`, `_busy()` → `/cache/abort`, battery). `_PS_KICK.set()` at pass start so wifi power save is off under the CDN burst. AM-38 is amended, not the code. | A4, Q20 |
| AM-68 | **AM-39's `vibb-warming` marker is BUILT, not assumed:** `radio.touch_warming()/warming()` (TTL 30 s, touched by the sweeper's 5 s poll); `btwatchd._radio_yield` holds on it BEFORE the 120 s starvation override, bounded by the TTL; the sidecar `wait_paging_clear()`s before EVERY `play`, not only the first (it reads the PAGING marker fine as `$RUN_USER`). | A11, Q21, Q6 |
| AM-69 | **idle.py, backup and the updater see the pass.** `daemon.status()` gains `warming` (from a cached `/soloist/health`); idle.py holds on it like `ssh_active` with a hard release at 25 min; `backup._box_busy` counts `warming`; the sidecar refuses to start and aborts a pass on a fresh `poweroff-imminent`. Live bug found on the way: the sidecar's `RUN_DIR` resolves to `/tmp` as `$RUN_USER` (`/run` not writable), so `_poweroff_imminent()` has NEVER fired on the box — fixed now: `Environment=VIBB_RUN=/run` in the unit, `RUN_DIR` from `vibb.paths`. | A5, Q19 |
| AM-70 | **The library stamps `_precache_done` only on `result: done`.** Today it stamps on the 202 and, for album/track/show kinds, `_precache_due` returns `prev is None` → an aborted album pass would never be re-POSTed. The sidecar keeps a FIFO of contexts (duplicates coalesced); POST answers `{queued|done}`; the sweeper waits for `warming == false` before the next POST and stamps from the ledger's result. Health shape: `warming: {uri, idx, n, started, until, result}`, `result` retained after the flag drops. | A6, Q24 |
| AM-71 | **The dwell is size-based, per-file, over the whole cache tree.** Directory mtime does not move on writes to an existing file, and the current track's bucket is unrelated to a prefetch's. So: snapshot the file set of `cache/*/` before `play`; watch the DELTA files with per-file sizes at 500 ms (≤256 scandirs, ms on a Zero). "Fetched" = a delta file ≥ 0.85 × duration_s × kbps × 125 B (96 kbps floor until the received bitrate is benched; a 131 168 B stub never passes) AND no growth for 2 s. Silence alone is not a signal: on a ~100 KB/s hotspot a 128 KiB block lands every 1.3 s. Already-cached = threshold met at t=0 → warmed at once, dwell 0 (this is also what makes a resume walk through warmed items cheap). A second delta file is the prefetch — ignored, it completes under the next item. Stall = no growth for 5 s below threshold → `radio.internet_up()` decides skip-track vs abort-pass; cap = max(30 s, expected_bytes / 50 KB/s). A track Soloist auto-skips (unavailable) → ledger state `unavailable`, not re-walked. | A9, Q10–Q13, draft Q1 |
| AM-72 | **Ledger keys.** Fingerprint = sha1 of the SORTED uris of the first window (ordered = never "done" under shuffle, which persists as a Soloist option) plus `complete = len(upcoming) < window`; `done` requires `complete`; an incomplete list re-queries `get_queue limit=0` when the walk reaches the window's end (lists > 80 finish over passes — AM-42 kept). `aborted_at` is DROPPED: the `warmed` set is the resume point. `self.build` is reset at every `start_child` (today it is set once per sidecar process, so AM-43's re-warm after a D1 swap never fired); ledger entries also carry a random `cache-id` written into `CACHE_DIRECTORY` at first child start (a restore, a cache wipe, a `-z` shrink change it → `warmed` emptied — the ledger rides in the backup's secret tier, the cache does not) and `z_mb`. A context verified `done` is re-verified at most once per 24 h; if every cached context is done-and-verified there is NO restart at all — a "done costs nothing" pass otherwise costs two restarts and the kid's paused session. | A7, Q14–Q18, draft Q2 |
| AM-73 | **Stores:** `$STATE_DIRECTORY/vibb/` — `meta.json` (uri → `entity_to_track` shape), `order-<sha1[:12]>.json` per context, `ledger.json` — written tmp + fsync + `os.replace` (the `content.py` pattern; `library.py`'s tmp+replace lacks the fsync), loaders tolerate truncation (`{}`), never next to Soloist's own `-D` files. `_listing_cache` becomes the in-memory mirror of the order store, keyed per uri (today: one context). | A8, Q28 |
| AM-74 | **`/context/tracks` from disk for a NON-active context** answers `ready: true` with `stale: true, remembered_at`; **`cached` becomes a COUNT** (the fork's dialect; the sidecar's bool made the PWA picker compute `True < 22` = "still filling" on every list — a live bug, fixed now). The Sonos hand-off sets `sonos_map_trusted = False` for any remembered listing (a same-length reorder would otherwise send the wrong `track_index`; the existing drift resync heals it); the picker uses it freely; adopt is harmless. | A-Q5, Q27, draft Q5 |
| AM-75 | **A bookmark for a track no longer in the list is rejected** (`accept_spot_bookmark` checks the uri against the remembered order) → clean start instead of a 300-skip walk landing at random. | Q8 |
| AM-76 | **Phone takeover during a pass aborts it:** `auth.is_active` flipping or a `context_changed` for a uri the pass did not request. | Q4 |
| AM-77 | **AM-42's `cache: N`, stated:** playlist/album/artist with N > 0 = the whole list under the ≤ 100/pass bound; show N = the newest N episodes (Soloist's `play show` order vs "newest" — bench line). The byte budget from `spotify_cache_gb` and a hotspot byte cap (~4 MB × items) go into the numbers. The POST carries `{uri, budget_s, max_items, max_bytes}`. **No battery threshold — owner 2026-09-05: the warm follows the same rule as every other sweep**, which today has none (`_cache_sweeper`: "Sweeps run on battery too: a box that mostly lives off the charger otherwise never syncs at all"); the only gates are `_busy()` and the pass budget. AM-42's "smaller on battery" is kept only as the budget `plugged_cached()` already picks. | C2, Q22, Q23 |
| AM-78 | **`spotify_cache_gb` does NOT restart anything live (draft Q4: no).** `go_unit_cmd("restart")` would restart the sidecar unit mid-play and mid-pass; `-z` is read at every child start, so the pass's own restarts and AM-52's idle restart already apply a PWA change. F7 stands; `resize_spotify_cache`'s early return is correct. | A-Q4, Q17 |
| AM-79 | **Bench lines before the walk is trusted** (one evening on the Zero, `bench/soloist_probe.py` + the cache listing): (a) does Soloist prefetch the next track while one plays; (b) does it keep fetching into a stream on `vibb_null` (`node.passive = true`) — file size after 10 s of `play` there; (c) the bitrate actually received (`spotify_bitrate` is go-librespot-only); (d) a fast skip through an already-whole file leaves no stub; (e) `play show`'s queue order; (f) whether the content-addressed cache survives a D1 build swap. | A13, Q2, Q12 |
| AM-80 | **Tests:** new `tests/soloist_warm.py` on the existing fakes, which grow: a scripted cache writer (128 KiB blocks at scenario rates: fast, slow-bursts 1.3 s/block, stall at 131 168 B, prefetch in another bucket, tiny track, already-cached, a second file created mid-fetch), `get_queue` honouring `limit`, a 10-entry broadcast on every skip, `TRACKS` longer than the window, trailing autoplay, an auto-skipping unavailable track, `context_changed` from "the phone" and `is_active` flips, fake-child modes `crash-after-play` and per-start numbered `argv.json` (asserting `-d vibb_null` then the real node, and `-z`), pw-dump graphs with/without `vibb_null`. Cases W1–W9: 202 + health shape + child on `vibb_null` + frozen-stopped `/status` with live `spotify_state`; ledger done/diff; play mid-pass = pause → restart real node → walk, and `/output` + `/pair` mid-pass abort first; `/cache/abort` idempotent; stop on autoplay/empty; stub never warmed, prefetch ignored, stall → result; crash mid-pass restarts on `vibb_null`; non-active listing from disk with `stale` + numeric `cached`; poweroff-imminent refused. New `tests/sweep_warm_gate.py` (sweeper vs a fake sidecar): 5 s poll, abort on gate flip, `_precache_done` only on done (an aborted album stays due). Extend `idle_shutdown` (hold + hard release), the precache gate (AM-45 default under `VIBB_GO_UNIT=vibb-soloistd`), the btwatchd yield test (warming marker), and the contract's TRANSLATE line for `/cache/download`. | A §4, Q §tests |
| AM-81 | **AM-79 (a)(b)(c) answered on the Zero, 2026-09-05 21:08, `soloist_probe.py --play`.** (b) **Soloist fetches the WHOLE track into `vibb_null`**: a manual child on `-d vibb_null` pulled 4 609 824 B for a 261 s track in 2.3 s (a 17 KB tail at 5.3 s), 141 kbps implied — the silent pass on the null sink stands. (a) Prefetch of the next item is exactly ONE block: a 131 168 B file appears alongside the main fetch and never grows — so F6's 'seven stubs from skips' were prefetched first blocks, not cut fetches; the dwell's size threshold already ignores them, and 'a second new file' must not be read as a parallel full fetch. (c) 160 kbps received (161 kbps on the real sink, 3 671 464 B / 183 s). Also seen: a 0-byte temp file that lives ~3 s — the watcher must tolerate files vanishing. The manual start used the unit's exact argv with only `-d` changed, which is what AM-62's override does. Remaining bench lines: (d) fast skip through a whole file, (e) `play show` order, (f) cache across a build swap — none gates the build. | probe A + B |
| AM-82 | **The first real pass on the Zero, 2026-09-05 23:04: `play failed: command requires authentication`.** The pass's first `play` went out a beat after the child on `vibb_null` accepted the socket, while the child was still "restoring session" (→ "logged in as …" ~1 s later). Two fixes in `soloistd.py`: (1) `_wait_ready(timeout, gen, abortable)` — the socket for THIS gen **and** `auth.logged_in` — before the pass's first command and before the Handler's abort-first play; (2) **the restore grace**: a fresh child answers `get_auth_state` `logged_in=false` while restoring, which derived to `needs-pair` for that second — and the daemon fast-fails a Spotify tap on `needs-pair` (AM-48), so a tap during ANY restart (pass start, pass end, output switch) would have said "not paired". Inside `RESTORE_GRACE_S` (8 s, env `VIBB_SOLOIST_RESTORE_GRACE_S`) a not-logged-in young child is `starting`; one re-derive at the grace's end makes a never-logged-in child honestly `needs-pair`. The fake now logs in 0.6 s after connect (`login_delay_s`), resets playback per child (a fresh process is idle), and its `pw-dump` can **follow** the newest child's `-d` node (`install_pw_dump("follow")`) — the static graph installed ahead of a pass had hidden both bugs. Pinned: warm W1 (no `needs-pair` across three children), sidecar 10 (grace → needs-pair at 1.5 s), sidecar 7b (lazy stream, wrong sink at play). | field + tests |
| AM-83 | **Second real pass on the Zero, 2026-09-05 23:26 — the pass runs end to end** (Edith 11 rows: 9 cached + 2 fetched in 40 s; Sverre 11 rows). Two findings. (1) **A cached track right after a fetched one read `stalled`** (twice, 6 s each): the previous file's late tail (AM-81: ~17 KB at ~5 s) lands during the next dwell, `seen_any` flips, nothing else grows, `STALL_S` expires. Dwell now ignores growth below `TAIL_MAX_B` (64 KB, under a block) on a file that existed at the baseline, and judges a file that DID grow by its whole size (a partial from an earlier play completes with a small delta). New files keep the exact one-block exclusion — a first block that never grows must still read as a stall (W6). Pinned: W10 (fetched, cached ×3 with a 17 KB tail, never `stalled`). (2) **`ledger: … forgetting 11 warmed rows (cache/size/build changed …)` at the END of both passes → `done (0 warmed)`.** The reset fires in `_warm_run`'s post-pass `_ledger_entry` because a stamp differs between the kid's child and the pass's child: `cache_id` (re-read per start from `$CACHE_DIRECTORY/vibb-cache-id` — gone if Soloist cleans unknown files in its cache dir, or never written) or `build` (per-child banner). Owner to read `ls -la /var/cache/vibb-soloist/`, the id file, and `/var/lib/vibb-soloist/vibb/ledger.json` before the fix. Until then a pass repeats its cached walk (~3 s/track) — no data loss. Also asked: the `mod.rt: Failed to connect to session bus … $DISPLAY` lines are PipeWire's `module-rt` in every client (`pw-dump`, the soloist child) falling back to RTKit over a session bus the unit user has none of; `LimitRTPRIO=95` on `vibb-soloistd.service` lets the client set its own priority and never asks RTKit — a real gain for the audio thread and the end of the noise (offered, not built). | field + tests |
| AM-84 | **AM-83 (2) answered by the box (2026-09-06 07:01): the `build` stamp, not the cache id.** `vibb-cache-id` is stable (`d2a3e639283c10eb`, written 23:04, Soloist leaves it alone); the ledger's `build` read `2026-09-06 07:00:23.796: soloist 1.3.8.13 build 1788609705 (20260905) (g5c3a2053ac) (linux/aarch64)` — the child's own log stamp is part of the banner line, so every child was a "new build" and `_ledger_entry` forgot the warmed rows at the end of every pass (the kid's child stamped at the start, the pass's child compared at the end). `_read_child` keeps the line from `soloist` on. The fake's banner now carries a live stamp in the Zero's shape, and the old parse fails W1 on the "second ask costs nothing" check (202 queued instead of 200 done) — proven before the fix. Sidecar case 1 pins the cleaned string. After the deploy the two lists walk once more (their ledger rows are empty, ~35 s each, zero bytes) and every later ask is an instant `done` with no restart. | box data + tests |
| AM-85 | **The queue window at a start is 10 upcoming on the Zero, not 80 (2026-09-06 07:19: `remembered 11 rows` for two lists the owner knows are longer; the spike's 80 was seen mid-list).** Three consequences fixed. (1) The walk **re-queries at the end of the remembered order** (`get_queue limit=0` from where it stands, `_remember` appends the unseen rows, `list grew to N rows`) and goes on to the context's real end — §4d.6's 'incomplete list re-queries at the window's end' was designed but never built. (2) `complete` is no longer `len(rows) < 80` at the start; it is set at the walk's END when every row is warm and the queue ended (autoplay/empty); a remembered order never proven complete makes the walk continue past its last row. (3) A **fresh start no longer reseeds** the remembered order to one window when the new window is a prefix of a longer known order (the kid pressing play after a pass kept the 11-row list, not the 22-row one) — a list edited at its head still reseeds, and the fingerprint then drops removed rows as before. `LEDGER_V = 2`: a row from an older sidecar keeps its warmed uris but owes ONE verify walk (`complete=False`, `verified_at=None`) — the two lists on the Zero finish over the next sweep instead of answering 'already done' for 24 h. Fake: `FAKE.window` caps upcoming per query. Pinned: W11 (3-row window: remembered 4, grew to 6, done, instant done, a fresh start keeps 6), W12 (pre-v2 row: 202 + walk with no fetch, then 200). **How 'already done' is decided (owner's question):** `warm_start` answers 200 from the ledger alone when result=done, complete, every remembered uri ∈ warmed ∪ unavailable, and `verified_at` < 24 h (`WARM_VERIFY_S`, AM-72). Nothing on the box can see a list change in Spotify without playing it: a verify = two child restarts + a silent play + a skip walk over warmed rows (~0.3 s each) — ~10–15 s per list. A library edit on the box wakes the sweep but does not force a verify. Owner to decide the verify cadence (24 h now; the 6 h sweep, or on every library edit of that entry, are the cheap alternatives). | field + tests |
| AM-86 | **The `mod.rt: Failed to connect to session bus: Unable to autolaunch a dbus-daemon without a $DISPLAY for X11` line (owner, 2026-09-06: every pw-dump, twice per soloist child).** It is libpipewire's `module-rt` inside every client: it wants SCHED_FIFO 88 / nice -11 for its data thread, tries `pthread_setschedparam` first and falls back to RTKit over the *session* bus only when that fails — an unprivileged systemd service has no session bus, so libdbus tries to autolaunch one and needs `$DISPLAY`. `vibb-soloistd.service` now carries `LimitRTPRIO=95` and `LimitNICE=-11`: the direct call succeeds, RTKit is never asked, the line is gone, and the soloist child's audio thread runs at the priority PipeWire wanted for it (the same thing RTKit would have granted). Root processes (vibbd's own pw-dump) never showed it. Pinned in `spotify_engine_toggle` (rendered unit). | field |
| AM-87 | **Sweep cadence and cost (owner, 2026-09-06).** (1) `WARM_VERIFY_S` 24 h → **5 h** (env `VIBB_WARM_VERIFY_S`): every 6 h sweep re-checks the lists for added/removed rows. (2) **One ownership per sweep**: the sweeper collects every due Spotify entry while it walks the library and POSTs them all before waiting (`_warm_entries`), the sidecar runs the queue under one ownership — one restart pair for all the lists, not one per list; results per uri come back in `health.warm_done` (reset when a pass thread starts) and the precache state is stamped per uri. (3) **A verify does not walk a list the start window shows whole**: with no targets, the walk to the last known row happens only when the order was never proven complete or the window does not reach that row (the Zero: 10 upcoming at a start — but 11 upcoming mid-list at 08:11, so the window is not a constant; the rule is window-agnostic). A skip over a warm row costs ~0.35 s (no dwell); a 25-row list verifies in ~9 s + the restart pair. Soloist cannot jump (play takes a uri only, PLAN-soloistd), so "skip every 80th" does not exist. (4) **Rows removed at the tail** are invisible to the start-window fingerprint: when the queue ends before the remembered order does, `_trim_after` cuts the order there and drops those rows from `warmed` (log `N trailing rows gone`). Pinned: gate 5 (two lists queued first, one poll loop, stamped per uri), W11 (a row appended behind the window is found by the verify, only that row fetched), W12 (the trim), W13 (two lists back to back = one restart pair). | owner + tests |
| AM-88 | **After a pass the PWA showed the pass's LAST track (owner, 2026-09-06 07:5x).** AM-64 froze `/status` while the pass owned the child and lifted the freeze at the restore — but the restored Soloist process *remembers its own last playback state*, i.e. the last track the pass played, so the live mirror said that. The fake had been made to start idle per child (2026-09-06 early), which hid it. Now: the pre-pass snapshot **outlives the restore** and stands until the box acts — any `/player/*` command, or a `playback_state` with `status: playing` while no pass owns the child (a phone transfer) — while `listing` and the Handler's warming test use `_warm_owned` instead of the snapshot. `is_active` is NOT a trigger (every fresh child re-announces it; that false trigger failed W1 once). The fake keeps playback across children (playing → paused at the restart) like the box. Pinned: W14 (kid plays, pauses; pass; `/status` = the kid's track, stopped; the listing live; a play makes it live), W1 (stopped after the pass). | field + tests |
| AM-89 | **Bench answer (Zero, 2026-09-06 ~09:00, `soloist_probe.py --queue-test`, Edith row 3 'Ferrari'): a track play + add_to_queue is a real substitute for the resume walk.** (a) `play <track uri>` → `track_changed` in **0.32 s**, status buffering; Soloist's context becomes the track uri itself. (b) The queue right after is empty (no radio yet). (c) `seek 45000` landed (45 019 ms) in 0.65 s. (d) **5 × `add_to_queue` in 0.28 s total** (~0.05 s each), `queue_changed` pushed for every one; the queue then reads the five as `[queue]` followed by the anchor track as `[context]`. (e) `skip_prev` from the first queued item goes **back to the anchor track** (position 0); rows BEFORE the anchor are not reachable by prev (previous stays empty). (f) When the queue drains Soloist returns to the anchor and **pauses** (end of the single-track context); one more skip_next starts autoplay radio. Consequences for a design: instant start on the right song and second, no lock held for N × 0.7 s, no passed tracks on the card; the rows after the anchor must all be queued (≤ 80 rows ≈ 4 s, after audio has started) or the kid hears a pause mid-list; the end of the list shows as 'anchor, paused' and must be presented as stopped; prev at the anchor restarts it (going further back means a new anchor play + requeue); shuffle must be ours (shuffle_context would shuffle a one-track context); origin/listing must key on OUR context (the playlist), not Soloist's; a list whose remembered order is not complete, or a target not in it, falls back to the context play + walk. The warm pass keeps the context play (it needs the context to discover rows). Owner to decide: build it ("hybrid start") or the async walk with a pending card. | probe |
| AM-90 | **Bench (Zero, 2026-09-06, `--burst-test`, 6+6 skips on Edith): a skip costs 0.15 s awaited AND 0.16 s in a burst** (6 sent in 2 ms, 6 `track_changed` at 0.15/0.33/0.50/0.65/0.81/0.95 s, every row in the history in order, landed on the expected row). Soloist serializes skips at its own pace; bursting gains nothing, and the walk over 21 rows is ~3.3 s, not 15–18 s. **The 18 s at 08:11:23 was not the walk**: the same context was already playing at row 0, so `play <context>` produced NO `track_changed` and `play()` waited its full 15 s for one before the snapshot (logged 08:11:39) and the 3 s walk; the player's retry then queued on the lock and, once through, sent `play <context>` again — which restarted the context from row 0 and walked again ('now playing: Golden', row 9, was that second walk's transient). Two small fixes, BUILT the same morning (owner's go), no rewrite: (1) `play()` with the context already active does not re-send `play <uri>` (a restart) and does not wait for a `track_changed` that cannot come — it walks from the current row to the target (forward with skip_next, or a short way back with skip_prev), and a retry that finds the target already current is a no-op; (2) the pending card (AM-88's sibling): while `pending_uri` is set, `/status` names the target from the metadata store, so the card never shows a passed row. With those, picking any row in a 22-row list is ≤ 3.5 s with the right art from the first second; an 80-row list ≤ 12 s. The hybrid start (AM-89) remains the only way to make it instant. Built shape: `_walk_to(uri, target)` — forward with skip_next; ≤ `WALK_BACK_MAX` (10) rows back = `seek 0` + skip_prev; farther back = restart at row 0 + forward; per-skip wait `WALK_STEP_WAIT_S` 3 s; a same-context `play` without a target still restarts (that is what a re-tap without a bookmark means) but waits only 3 s for the track_changed. `status()` names the pending target from the metadata store. Pinned: sidecar 11 (no `play <uri>` on a same-context walk, 3 skips in < 6 s with a 0.4 s fake skip, the card = target while walking, seek 0 + 2 × skip_prev for two rows back, a retry moves nothing, 3 rows back over a WALK_BACK_MAX of 2 restarts and walks 1 forward). | probe + log |
| AM-91 | **Boot log review (owner, 2026-09-06 10:12 boot).** BT connect itself is 1 s from the kick. What is around it: (1) NM starts 19 s late — netplan's hook regenerates three times during NM's own startup, each with a 3–4 s `daemon-reload`; wifi settles at boot+35 s and btwatchd's boot-window hold (WIFI_GATE / wifi_settled, the 2026-07-18 deauth incident) follows it. NM's plugins are plain `keyfile`; the netplan coupling comes from `/run/NetworkManager/conf.d/netplan.conf`, no config switch — PARKED (removing netplan is a platform change). The hand-written `50-vibb-wifi.yaml` is GONE: NM migrated it into `90-NM-<uuid>.yaml`, rewritten at every boot — the 0-byte failure mode stands; owner leaning to a vibb-owned wifi copy + boot self-heal (design pending). Owner rejected 'wifi waits for BT' (often no BT at all, Sonos). (2) **Both PipeWire daemons ran at normal priority** (`RTKit does not give us MaxRealtimePriority`): `LimitRTPRIO=95` + `LimitNICE=-11` on `pipewire.service` and `wireplumber.service` — the AM-4 escalation, taken. (3) `mpris-proxy.service` (Debian's session bridge) started in the owner's ssh session and registered a second AVRCP player next to vibb-mpris: masked globally with the other session units. (4) The boot self-test read FAIL-SAFETY (`probe-missing`, `pinned-not-linked`, `followed-default`): the pw-cat probes need > 1 s to appear under the boot storm. On this branch the verdict feeds only `/status.audio_policy` and a log line (AM-58's one-cap change left `cap_everywhere()` with no caller) — no volume effect; the owner questions the test's value; decision pending (fix the 1 s waits and run it off the boot storm, or drop it). Pinned: audio_stack_toggle (limits in both units, mpris-proxy in the global masks). | log + owner |
| AM-92 | **The policy self-test runs on request only (owner, 2026-09-07: "nr 2").** Gone: the boot run, the once-a-minute cookie watch (re-run on a PipeWire restart), and the run after a bt.py recovery (AM-8). Kept: `policy_selftest()` itself, `POST /audio/selftest` (202, off the request thread, single-flight) and `/status.audio_policy` (`pending` until someone runs it). Why: under the boot storm the 1 s probe waits read FAIL-SAFETY falsely, the verdict feeds nothing but that status field (AM-58 left `cap_everywhere()` without a caller), it cost ~6 s of CPU while vibb-ui came up, and after a recovery it competed with the reconnect + resume. It stays a guard to run by hand after a WirePlumber/PipeWire upgrade: `cd ~/vibb/pi && sudo python3 -c 'from vibb import audio; print(audio.policy_selftest())'`. A PWA button is a later ask. Pinned: audio_policy_daemon 2 (no run after a recovery), 4 (no watcher thread). | owner |
| AM-93 | **btwatchd's boot hold protects wifi, not BT — so it holds only while wlan0 is actually being set up (owner, 2026-09-07).** The two radio incidents are different things: the BT firmware crashes (hardware error 0x00) came from A2DP streaming under wifi load (scan/unblock mid-stream, the Peripheral-role Skoda case) and are handled by the A2DP quiesce before an NM scan and by `recover()`; the 2026-07-18 boot incident was blind BT pages in a 5 s loop landing in wifi's association/DHCP window and deauthing WIFI (reason=6) — wifi was the victim, BT was fine, and the page loop is already capped by BOOT_FAIL_LIMIT. The old rule ('hold inside the 45 s gate until wifi has a route') made BT wait for NetworkManager, which on Trixie starts ~20 s late (AM-91's netplan rounds): headset at boot+36 s. New rule: `radio.wifi_assoc_in_flight()` — wlan0 up/dormant/unknown WITHOUT a default route = the fragile window, hold; wlan0 down (nobody has touched the radio) or routed = page now. Kicks still bypass; the offline cabin boot (wlan0 scanning forever) still waits ≤ 45 s. Expected: headset at ~boot+18 s (btwatchd up) instead of +36 s. Residual: a single 1–3 s page overlapping NM's first association attempt → NM retries by itself, btwatchd (now connected) does not page again. Field check owed: the next boot's timestamps for `btwatchd: connecting` vs `wpa_supplicant: Trying to associate`. Pinned: radio_markers 10, bt_radio_yield 1b. | owner + tests |
| AM-94 | **Soloist feature requests — the go-librespot parity register (owner asked 2026-09-07).** Channels per the Soloist page: client issues → `github.com/spotify/soloist/issues`; questions → the Spotify Developer Community thread. Ranked by what each would remove on the box. (1) **`play` with a start offset** (`skip_to` uid/uri/index + `position_ms`, or a `jump_to` command): today a resume or a song pick is a skip walk at 0.15 s/row under the shroud (AM-90) and 'instant' needs the hybrid track-play + queue (AM-89) with its prev/shuffle/listing costs. (2) **The whole context tracklist on demand** (`get_context` or `get_queue` without the 10-row start window and the 10-deep `previous` cap): today lists are remembered across sessions and completed by the warm pass's walk (AM-60/85). (3) **A playlist revision/snapshot id in the context entity**: today a list change is detected only by playing it (fingerprint of the start window, tail trim on the walk, AM-85/87). (4) **A prefetch/cache command** (`prefetch {uri}` for a track or a context, plus a `get_cache` query of what is whole on disk): today warming is a silent play on a null sink with per-track size dwell (§4d) and cache knowledge is inferred from file growth (AM-71/81/83). (5) **Runtime output switch** (`set_output {node}`): today `-d` is a start argument, so every output switch and every warm pass restarts the child (AM-62/64/88 and the restore grace of AM-82). (6) **`set_autoplay {enabled}`**: today the walk stops at the autoplay boundary and a drained queue lands on the anchor (AM-41/89). (7) **A bitrate/quality setting**: today quality follows the account's download setting (PLAN-soloistd) — a hotspot-friendly low rate is not selectable. (8) **`auth_state.username`** and a documented bad-key line: today synthesized / the widest net (AM-47/48). (9) **Offline playback from the cache** documented (keys/licences): today unknown — the wifi-off test is owed. Nice-to-have: a `pending`/`loading` status while a command is in flight, and `queue_changed` on context load without a query. Not needed: bursts of skips (AM-90), a second WS client (already accepted), verbose logging (exists). | owner |
| AM-95 | **Migration QA for the son's production box (main: bluealsa + go-librespot + PiSugar, months of state) → `pipewire --soloist` (owner asked 2026-09-07; two review agents + verification).** Verdict: NOT yet — six code fixes and one rehearsal first, then cut over on the charger with the headset off. **Blockers (verified in code):** (1) **Rollback leaves `/etc/asound.conf` in PipeWire format** — `bt.py` `_route_alsa()` returns early when the MAC is already in the file, and the PipeWire text carries `# bt speaker <MAC>`; install's placeholder guards grep for `vibb_bt`/`pcm.vibb_local`, present in both formats → both pcms point at a masked server, silent box on both outputs. Fix: the bluealsa branch of `route()` rewrites unconditionally. (2) **A live flip quiesces nothing** — `mask --now bluealsa` runs in step 4, the daemon/btwatchd restart in step 6: a playing mpv hits the advance storm and rewrites bookmarks, go-librespot's ALSA handle dies, the OLD btwatchd may open the HAT directly against WirePlumber. Fix: install stops vibb-daemon + vibb-bt-reconnect before the flip (or the owner does, headset off). (3) **`local_fallback_cap` (main default 35, HAT-only) is gone**: dead-headphones fallback onto the HAT now lands at `volume_cap` (default 100) — the loudest event the box can produce, in the dark. Owner decision AM-58 was taken on a fresh box; on the old box the 35 was doing work. **Owner 2026-09-07: the HAT cap is removed on purpose and stays removed for the production box too — not a blocker, `volume_cap` is the one ceiling.** (4) **Sonos hand-off and the song picker are empty for every Spotify card never played under soloist**: `listing()` serves the remembered store, empty after install → `nothing-to-play`; existing entries carry an explicit `cache: 0` (the new default 1 applies only to absent keys) so no pass ever remembers them. Fix: at cutover flip Spotify entries to `cache: 1` and let the first sweep run on the charger, or a live fallback in `listing()`. **Must-fix-before:** (5) `/etc/vibb/audio-stack` is written in `audio_stack_resolve` BEFORE apt/apply — an aborted flip leaves a lying file (readers: `audio.stack()`, route, extra.sh) and, on the verification-failure path, Debian's user-session PipeWire unmasked; the engine file is written last, asymmetric. Fix: write after a successful apply. (6) **The bookmark rejection rule (AM-75) fires on a PARTIAL remembered list**: after the first play the store holds one ~21-row window; a bookmark at row 40 reads 'no longer in the list' → clean start from row 0. Fix: apply only when the order is `complete` (expose it from `listing()`). (7) The go-librespot-era `spotify-precache.json` says `queued` for albums/shows → never warmed under soloist; delete it at cutover (one full pass follows). (8) The first sweep after cutover runs every due list (empty ledger): 20 min/list budget, idle held ≤ 25 min, wifi PS off, no battery guard (same rule as the podcast sweep, AM-77) — cut over on the charger. (9) Two 20 GB caches on one card: go-librespot's `/var/lib/vibb/spotify-cache` is neither pruned nor reported under soloist; no free-space guard anywhere. Fix: remove it at cutover (or prune under `--soloist`), consider `spotify_cache_gb`. (10) The first Soloist build fetch is silently SKIPPED while the headset is connected (`_box_busy` includes `bt_connected`; no `--force` on the unit) and the sidecar then reads `offline`, not 'no binary'. Fix: the install path fetches with force when no binary exists. (11) Backup misses `/etc/vibb/soloist.env` and the sidecar store → a restore lands in needs-key. Fix: add to `_secret_files()`. (12) No OS/glibc check: WirePlumber 0.5 config + Soloist glibc ≥ 2.38 = Trixie only; the production box is Trixie per the rig memory — confirm with `os-release`. **Worth knowing:** offline rollback needs apt-free prerequisites (swig/liblgpio-dev, go-librespot version file, venv imports) — pre-install them while online; a headset connected through the flip recovers via the nudge (~15 s silent); go-librespot is disabled not masked (two Connect devices between steps 5 and 6); step 7 polls go-librespot's port for 20 s under soloist; `--librespot` on PipeWire (s6) is unverified — roll back both toggles together; `sonos_map_trusted=False` on a stale listing also disables the same-card shortcut and shows the speaker's DIDL title. **Main's path is not untouched by the branch** (the owner's rule): NEW-2 pause-confirm + try-restart in player.py, the BT gate shadow compare, AM-93's boot hold, bluealsa-aplay/mpris-proxy masks, `_go_volume_cap` after a live reopen, the hotspot PSK, swig in PKGS, the Speaker-limit menu removed — all intended, listed here so they are not surprises. **The plan's own gate (QA §4)** is unmet: no 7-day baseline, rollback drill never run (it would fail on (1)), soak 2 days on the daughter's box, B10 not run, the wifi self-heal (AM-91) not built. Recommended order: fix 1–2, 5–7, 10–11 → rehearse the rollback on the daughter's box offline → pre-flight the production box → cut over on the charger, headset off, first sweep to completion → kid tests (Sonos hand-off of an unplayed card, deep-bookmark resume, dead-headphones fallback level). | review |
| AM-96 | **AM-95's fix set, BUILT (owner's go, 2026-09-07).** (1) `bt._route_alsa` is idempotent per FORMAT (`type pipewire` / `type bluealsa` marker beside the MAC): a rollback rewrites the file — `asound_rollback_rewrite`. (2) `install.sh` stops vibb-daemon + vibb-bt-reconnect before `audio_stack_apply` when the recorded stack differs from the one asked for (they come back at step 6) — `install_unit_order`. (3) `/etc/vibb/audio-stack` is written by `_as_record_stack` at the END of a successful apply (both branches); `audio_stack_resolve` no longer writes, `audio_stack_peek` honours the in-process resolution, `audio_stack_recorded` reads the file only — `audio_stack_toggle` 13. (4) `listing()` exposes `complete`; `accept_spot_bookmark` rejects only against a complete list — `spot_bookmark_complete_list`. (5) `_se_migrate_from_golibrespot` runs once when the recorded engine is not soloist: drops `spotify-precache.json`, removes `/var/lib/vibb/spotify-cache` (size logged), flips Spotify entries `cache: 0 → 1` in library.json (others untouched) so the first sweep warms and remembers every list. (6) With no `/usr/local/bin/soloist` the soloist apply runs the updater with `--force` once (`VIBB_SOLOIST_UPDATER` overrides it in tests) — `spotify_engine_toggle` 6. (7) Backup: `soloist.env` and Soloist's own data-dir files are SECRET (ws.* excluded), the sidecar store under `/var/lib/vibb-soloist/vibb` is PROGRESS — `backup_soloist_files`. Not built (owner's calls): the HAT cap stays removed (AM-58); the wifi self-heal (AM-91) still awaits the storage yes. Next: the rollback drill on the daughter's box, offline (`sudo ./pi/install.sh --bluealsa --librespot`, then `grep -c 'type bluealsa' /etc/asound.conf` = 1, `bluealsa-aplay -L` lists the headset, back with `--pipewire --soloist`). | built |

**Fixed tonight, ahead of 4d, because they are live bugs:** AM-69's `RUN_DIR`/`VIBB_RUN` (the sidecar never saw `poweroff-imminent`), AM-72's `self.build` reset (D1 re-warm key), AM-74's `cached` count (the PWA picker's "still filling").
